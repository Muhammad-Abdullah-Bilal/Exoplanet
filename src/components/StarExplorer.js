...
import Enhanced3DPlanet from './Enhanced3DPlanet';
...
const StarExplorer = ({ isOpen, onClose }) => {
  ...
  // Star catalog with realistic data
  const starCatalog = [
    {
      id: 'kepler-452',
      name: 'Kepler-452',
    ...
    starCatalog.forEach(star => {
      ...
      if (star.planets && star.planets.length > 0) {
        star.planets.forEach((planet, index) => {
          const orbitRadius = starRadius + 15 + (index * 8);
          const planetSize = 2;
          
          ctx.fillStyle = planet.habitableZone ? '#4caf50' : planet.color;
          ctx.beginPath();
          ctx.arc(
            star.coordinates.x + Math.cos(Date.now() * 0.001 + index) * orbitRadius,
            star.coordinates.y + Math.sin(Date.now() * 0.001 + index) * orbitRadius,
            planetSize,
            0,
            Math.PI * 2
          );
  ...
  const handleCanvasClick = (event) => {
    ...
    const clickedStar = starCatalog.find(star => {
      const distance = Math.sqrt(
        Math.pow(clickX - star.coordinates.x, 2) + 
        Math.pow(clickY - star.coordinates.y, 2)
      );
      return distance <= Math.max(3, star.size * 8) + 10;
    });
  ...
  const handleCanvasMouseMove = (event) => {
    ...
    const hoveredStar = starCatalog.find(star => {
      const distance = Math.sqrt(
        Math.pow(mouseX - star.coordinates.x, 2) + 
        Math.pow(mouseY - star.coordinates.y, 2)
      );
      return distance <= Math.max(3, star.size * 8) + 10;
    });
                  ...
                  >
                    <div className="planet-preview">
                      <Enhanced3DPlanet
                        color={planet.color}
                        size="small"
                        hasRings={planet.type === 'gas-giant'}
              ...
              <div className="planet-details-grid">
                <div className="planet-visual">
                  <Enhanced3DPlanet
                    ...
                    size="large"
                    hasRings={selectedPlanet.type === 'gas-giant'}
                    hasAtmosphere={selectedPlanet.hasAtmosphere}
